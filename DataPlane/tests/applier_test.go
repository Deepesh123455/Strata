...
func TestPersistenceRoundTrip(t *testing.T) {
	...
	if _, err := wal.Replay(path, func(_ uint64, p []byte) error { return applier.ApplyRecord(c1, p) }); err != nil {
	...
	lastSeq, err := applier.LoadFromWAL(path, c2)
	if err != nil {
...
func TestTornTailTruncated(t *testing.T) {
	...
	c2 := engine.NewPowerhouseCache(0)
	if _, err := applier.LoadFromWAL(path, c2); err != nil {
	...
	if info.Size() != 17+1+1+2+2 /* hdr + $1\r\nk\r\n style record */ {
		...
		if _, err := applier.LoadFromWAL(path, engine.NewPowerhouseCache(0)); err != nil {
